...
파라미터 작업 - scriptableobject표준 (MonsterData.cs 참고)
(EnemySpawner.cs에서 활용됨)을 보고
개인 작업 영역에 필요한 scriptableObject 작성하기. 

재준님 - 플레이어 마우스 상시바라보는거, 카메라설정
          - 무기 파라미터 , (축복) 사출, 주변 회전,지점 생성형 메커니즘 

성호님 - 아이템 파라미터 , 아이템 드랍을 드랍할 아이템 풀에서 랜덤하게 드랍하는 기능, 플레이어가 아이템과 충돌시 -> 충돌아이템 종류 식별 -> 해당종류의 아이템에 맞는 이벤트 발생

정환님 - 캐릭터 기능 ( 공격, 상호작용등 의 StateMachine 설계 ), 몬스터 기능, 몬스터 상호작용 기능구현

박지민 - 건물 파라미터, 건물기능들 마무리, 게임매니저 역할정의, 기능구현
