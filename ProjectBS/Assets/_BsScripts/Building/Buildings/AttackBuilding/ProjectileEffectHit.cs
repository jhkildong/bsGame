...
public class ProjectileEffectHit : MonoBehaviour , ISetProjectileStats
{
    ...
    void OnTriggerEnter(Collider other)
    {
        if(other.gameObject != null && (attackableLayer.value & (1 << other.gameObject.layer)) != 0)
        {
            ...
            if (!canPenetrate) EffectPoolManager.Instance.ReleaseObject<ProjectileEffectHit>(gameObject); // 관통불가능한 경우 투사체 비활성화, 매니저로 반환
            else if (canPenetrate && orgPenetrateCount > 0) // 관통가능한데, 관통횟수제한이 있는경우
            {
                curPenetrateCount--; //남은 관통횟수 차감
                if (curPenetrateCount < 0) EffectPoolManager.Instance.ReleaseObject<ProjectileEffectHit>(gameObject); // 남은 관통횟수가 0미만일때 매니저로 반환
            }
        }
    }
