...
public abstract class Building : MonoBehaviour , IDamage, IHealing
{
    ...
    [SerializeField] protected Building _nextUpgrade;  //다음 업그레이드 건물
    ...
    */
    ...
    protected virtual void Update()
    {
        ...
        */
        ...
        if(iscompletedBuilding&&Input.GetKeyDown(KeyCode.U)) 
        {
            //다음 업그레이드가있고, 재화가 충족될시
            Debug.Log(GameManager.Instance.CurWood());
            Debug.Log(_nextUpgrade.Data.requireWood);
            if(_nextUpgrade != null&& GameManager.Instance.CurWood() >= _nextUpgrade.Data.requireWood && GameManager.Instance.CurStone() >= _nextUpgrade.Data.requireStone
                && GameManager.Instance.CurIron() >= _nextUpgrade.Data.requireIron)
            {
                //건물 교체
                Upgrade();
