    ...
    protected virtual void Update()
    {
        ...
        */
        ...
        if(iscompletedBuilding&&Input.GetKeyDown(KeyCode.U)) 
        {
            ...
            Building bd = _nextUpgrade.GetComponent<Building>();
            Debug.Log(bd.Data.requireStone);
            if (_nextUpgrade != null&& GameManager.Instance.CurWood() >= bd.Data.requireWood && GameManager.Instance.CurStone() >= bd.Data.requireStone
                && GameManager.Instance.CurIron() >= bd.Data.requireIron)
            {
                //건물 교체
                Upgrade();
